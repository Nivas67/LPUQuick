...
function switchView(viewName) {
    ...
    const titles = {
        ...
        'advertisements': 'Advertisements & Promotional Banners',
    ...
    else if (viewName === 'advertisements') loadBannersAdmin();
...
async function refreshCurrentView() {
    ...
    try {
        ...
        if (activeView === 'dashboard') {
            ...
            promises.push(loadBannersAdmin());
...
// ============================================================
// ADVERTISEMENTS & PROMOTIONAL BANNERS MANAGEMENT
// ============================================================
let adminBannersList = [];
let adminBannerSettings = { autoplay_delay: 4500, autoplay_enabled: true };
...
async function loadBannersAdmin() {
    try {
        ...
        if (data.success) {
            ...
            const badge = document.getElementById('nav-banners-badge');
            if (badge) {
                const activeCount = adminBannersList.filter(b => b.is_active !== false).length;
                badge.textContent = activeCount;
            }

            // Sync settings card
            const delaySlider = document.getElementById('banner-delay-slider');
            const delayDisplay = document.getElementById('banner-delay-display');
            const autoplayToggle = document.getElementById('banner-autoplay-toggle');
            const speedIcon = document.getElementById('banner-speed-icon');

            const currentDelaySec = (adminBannerSettings.autoplay_delay || 4500) / 1000;
            if (delaySlider) delaySlider.value = currentDelaySec;
            if (delayDisplay) delayDisplay.textContent = `${currentDelaySec} seconds`;
            if (autoplayToggle) autoplayToggle.checked = adminBannerSettings.autoplay_enabled !== false;
            if (speedIcon) speedIcon.style.animationDuration = `${currentDelaySec}s`;
        ...
        showToast('Failed to load promotional banners: ' + err.message, 'warning');
    }
}
window.loadBannersAdmin = loadBannersAdmin;

function highlightActivePresetButton(sec) {
    document.querySelectorAll('.delay-preset-btn').forEach(btn => {
        const preset = parseFloat(btn.dataset.preset);
        if (Math.abs(preset - sec) < 0.1) {
            btn.classList.add('border-emerald-500', 'bg-emerald-50', 'text-emerald-800');
            btn.classList.remove('border-[#DADCE0]', 'text-[#3c4043]');
        } else {
            btn.classList.remove('border-emerald-500', 'bg-emerald-50', 'text-emerald-800');
            btn.classList.add('border-[#DADCE0]', 'text-[#3c4043]');
        }
    });
}

function handleDelaySliderChange(val) {
    const sec = parseFloat(val);
    const delayDisplay = document.getElementById('banner-delay-display');
    if (delayDisplay) delayDisplay.textContent = `${sec} seconds`;

    const speedIcon = document.getElementById('banner-speed-icon');
    if (speedIcon) speedIcon.style.animationDuration = `${sec}s`;

    highlightActivePresetButton(sec);
}
window.handleDelaySliderChange = handleDelaySliderChange;

function setDelayPreset(sec) {
    const delaySlider = document.getElementById('banner-delay-slider');
    if (delaySlider) {
        delaySlider.value = sec;
        handleDelaySliderChange(sec);
    }
}
window.setDelayPreset = setDelayPreset;

async function saveBannerSettings() {
    const btn = document.getElementById('btn-save-banner-settings');
    const delaySlider = document.getElementById('banner-delay-slider');
    const autoplayToggle = document.getElementById('banner-autoplay-toggle');

    const delaySec = parseFloat(delaySlider?.value || 4.5);
    const autoplayEnabled = Boolean(autoplayToggle?.checked);
    ...
    try {
        const res = await fetchWithTimeout('/api/banners/admin/settings', {
            ...
            headers: getAuthHeaders(),
            body: JSON.stringify({
                autoplay_delay: Math.round(delaySec * 1000),
        ...
        showToast('Error saving settings: ' + err.message, 'warning');
    } finally {
        if (btn) {
            btn.disabled = false;
            btn.innerHTML = '<span class="material-symbols-outlined text-sm">save</span><span>Save Timing</span>';
        }
    }
}
window.saveBannerSettings = saveBannerSettings;

function filterBannersAdmin(status) {
    currentBannerFilter = status;
    document.querySelectorAll('.banner-filter-btn').forEach(btn => {
        btn.classList.toggle('active', btn.dataset.status === status);
        btn.classList.toggle('bg-[#3c4043]', btn.dataset.status === status);
        btn.classList.toggle('text-white', btn.dataset.status === status);
        btn.classList.toggle('border', btn.dataset.status !== status);
        btn.classList.toggle('border-[#DADCE0]', btn.dataset.status !== status);
        btn.classList.toggle('text-[#5c5f60]', btn.dataset.status !== status);
    });
    renderBannersGrid();
}
window.filterBannersAdmin = filterBannersAdmin;

function renderBannersGrid() {
    const container = document.getElementById('banners-grid');
    const countBadge = document.getElementById('banner-count-badge');
    if (!container) return;

    let items = [...adminBannersList].sort((a, b) => (a.display_order || 0) - (b.display_order || 0));

    if (currentBannerFilter === 'active') {
        items = items.filter(b => b.is_active !== false);
    } else if (currentBannerFilter === 'paused') {
        items = items.filter(b => b.is_active === false);
    }

    if (countBadge) countBadge.textContent = adminBannersList.length;

    if (items.length === 0) {
        container.innerHTML = `
            <div class="col-span-full p-12 text-center text-[#5c5f60] bg-white rounded-2xl border border-dashed border-[#DADCE0]">
                <span class="material-symbols-outlined text-4xl text-slate-400">campaign</span>
                <p class="font-bold text-sm text-[#181c1f] mt-2">No banners found</p>
                <p class="text-xs text-[#5c5f60] mt-0.5">Click "Add New Poster / Banner" above to create one.</p>
            </div>
        `;
        return;
    }
    ...
    const gradientClasses = {
        ...
        purple: 'from-purple-950 via-purple-900 to-purple-700',
        amber: 'from-amber-950 via-amber-800 to-amber-600',
        cyan: 'from-cyan-950 via-cyan-900 to-cyan-700',
        rose: 'from-rose-950 via-rose-900 to-rose-700',
        slate: 'from-slate-950 via-slate-900 to-slate-800'
    };

    container.innerHTML = items.map((b, idx) => {
        const isActive = b.is_active !== false;
        const gradClass = gradientClasses[b.gradient] || gradientClasses.emerald;
        const hasImg = Boolean(b.image_url);

        return `
            <div class="bg-white rounded-2xl border border-[#DADCE0] overflow-hidden shadow-sm hover:shadow-md transition-all flex flex-col justify-between">
                <!-- Banner Live Visual Preview Card -->
                <div class="relative w-full h-44 overflow-hidden bg-gradient-to-r ${gradClass} flex items-center justify-center">
                    ${hasImg ? `
                        <img src="${b.image_url}" alt="${escapeHtml(b.title || 'Banner')}" class="absolute inset-0 w-full h-full object-cover z-0" onerror="this.classList.add('hidden')">
                    ` : ''}

                    ${!b.is_full_poster ? `
                        <div class="absolute inset-0 ${hasImg ? 'bg-gradient-to-r from-black/80 via-black/40 to-transparent' : ''} p-5 flex flex-col justify-between text-white z-10">
                            <div>
                                <div class="flex items-center gap-1.5 flex-wrap mb-1.5">
                                    ${b.pill ? `<span class="text-[9px] font-black px-2.5 py-0.5 rounded-full bg-black/40 backdrop-blur-sm border border-white/20 text-white">${escapeHtml(b.pill)}</span>` : ''}
                                    ${b.badge ? `<span class="text-[9px] font-black px-2.5 py-0.5 rounded-full bg-white/20 backdrop-blur-sm border border-white/30 text-white">${escapeHtml(b.badge)}</span>` : ''}
                                </div>
                                <h4 class="text-base font-black tracking-tight leading-tight line-clamp-1 drop-shadow-sm">${escapeHtml(b.title || 'Promotional Slide')}</h4>
                                <p class="text-xs text-white/90 line-clamp-2 mt-1 leading-relaxed">${escapeHtml(b.subtitle || '')}</p>
                            </div>
                            <div class="pt-2">
                                <span class="text-[11px] font-black px-3 py-1 rounded-full bg-white text-slate-900 shadow-sm inline-block">${escapeHtml(b.link_text || 'Order Now')}</span>
                            </div>
                        </div>
                    ` : `
                        <div class="absolute top-3 left-3 z-10">
                            <span class="text-[10px] font-black px-2.5 py-0.5 rounded-full bg-black/60 text-white backdrop-blur-sm border border-white/20">Full Poster Graphic</span>
                        </div>
                    `}

                    <!-- Display Order Badge -->
                    <div class="absolute top-3 right-3 z-20 flex items-center gap-1.5">
                        <span class="text-[10px] font-black px-2.5 py-0.5 rounded-full bg-black/70 text-white backdrop-blur-sm shadow-sm">
                            #${b.display_order || (idx + 1)}
                        </span>
                        <span class="text-[10px] font-bold px-2 py-0.5 rounded-full ${isActive ? 'bg-emerald-500 text-white' : 'bg-rose-500 text-white'} shadow-sm">
                ...
                <div class="p-4 bg-white border-t border-[#DADCE0] flex flex-wrap items-center justify-between gap-3 text-xs">
                    <div class="flex items-center gap-2">
                        <!-- Move Up / Down -->
                        <div class="flex items-center border border-[#DADCE0] rounded-lg overflow-hidden">
                            <button onclick="reorderBanner('${b.id}', -1)" title="Move slide forward in sequence" class="p-1.5 hover:bg-[#f1f4f7] text-[#5c5f60] transition-colors">
                                <span class="material-symbols-outlined text-[16px]">arrow_upward</span>
                            </button>
                            <button onclick="reorderBanner('${b.id}', 1)" title="Move slide backward in sequence" class="p-1.5 hover:bg-[#f1f4f7] text-[#5c5f60] transition-colors border-l border-[#DADCE0]">
                                <span class="material-symbols-outlined text-[16px]">arrow_downward</span>
                            </button>
                        </div>

                        <!-- Active Toggle -->
                        <label class="flex items-center gap-1.5 cursor-pointer font-semibold text-[#181c1f]">
                            <input type="checkbox" ${isActive ? 'checked' : ''} onchange="toggleBannerStatus('${b.id}', ${isActive})" class="w-3.5 h-3.5 rounded text-emerald-600 focus:ring-emerald-500 border-[#DADCE0]">
                            <span class="text-[11px]">${isActive ? 'Visible' : 'Hidden'}</span>
                        </label>
                    </div>

                    <!-- Actions: Edit & Delete -->
                    <div class="flex items-center gap-2">
                        <button onclick="openBannerModal('${b.id}')" class="px-3 py-1.5 rounded-lg border border-[#DADCE0] hover:bg-[#f1f4f7] text-[#262a2d] font-bold text-xs flex items-center gap-1 transition-colors">
                            <span class="material-symbols-outlined text-sm">edit</span>
                            <span>Edit</span>
                        </button>
                        <button onclick="deleteBanner('${b.id}')" class="px-2.5 py-1.5 rounded-lg border border-rose-200 text-rose-600 hover:bg-rose-50 font-bold text-xs flex items-center gap-1 transition-colors">
...
function openBannerModal(bannerId = null) {
    ...
    const titleEl = document.getElementById('modal-banner-title');
    const deleteBtn = document.getElementById('btn-modal-delete-banner');
    if (!modal) return;

    modal.classList.remove('hidden');

    if (bannerId) {
        const b = adminBannersList.find(item => item.id === bannerId);
        if (!b) return;

        titleEl.textContent = 'Edit Promotional Banner';
        if (deleteBtn) deleteBtn.classList.remove('hidden');

        document.getElementById('form-banner-id').value = b.id;
        document.getElementById('form-banner-heading').value = b.title || '';
        document.getElementById('form-banner-subtitle').value = b.subtitle || '';
        document.getElementById('form-banner-pill').value = b.pill || '';
        document.getElementById('form-banner-badge').value = b.badge || '';
        document.getElementById('form-banner-cta-text').value = b.link_text || 'Order Now';
        document.getElementById('form-banner-cta-url').value = b.link_url || '#/categories';
        document.getElementById('form-banner-order').value = b.display_order || 1;
        document.getElementById('form-banner-active').checked = b.is_active !== false;
        document.getElementById('form-banner-full-poster').checked = Boolean(b.is_full_poster);
        document.getElementById('form-banner-image').value = b.image_url || '';

        // Select radio gradient
        const radio = document.querySelector(`input[name="banner-gradient"][value="${b.gradient || 'emerald'}"]`);
        if (radio) radio.checked = true;

        toggleFullPosterMode(Boolean(b.is_full_poster));
        updateBannerPreviewImage(b.image_url || '');
        updateBannerLivePreview();
    } else {
        titleEl.textContent = 'Add Promotional Banner';
        if (deleteBtn) deleteBtn.classList.add('hidden');

        document.getElementById('form-banner-id').value = '';
        document.getElementById('form-banner-heading').value = '';
        document.getElementById('form-banner-subtitle').value = '';
        document.getElementById('form-banner-pill').value = '⚡ FLASH DEAL';
        document.getElementById('form-banner-badge').value = '🎉 20% OFF';
        document.getElementById('form-banner-cta-text').value = 'Shop Now';
        document.getElementById('form-banner-cta-url').value = '#/categories';
        document.getElementById('form-banner-order').value = (adminBannersList.length + 1);
        document.getElementById('form-banner-active').checked = true;
        document.getElementById('form-banner-full-poster').checked = false;
        document.getElementById('form-banner-image').value = '';
        document.getElementById('form-banner-file').value = '';

        const radio = document.querySelector('input[name="banner-gradient"][value="emerald"]');
        if (radio) radio.checked = true;

        toggleFullPosterMode(false);
        clearBannerImage();
        updateBannerLivePreview();
    }
}
...
function toggleFullPosterMode(isFull) {
    const textContainer = document.getElementById('banner-text-fields-container');
    const previewOverlay = document.getElementById('form-banner-preview-overlay');
    if (textContainer) {
        if (isFull) {
            textContainer.classList.add('opacity-40', 'pointer-events-none');
        } else {
            textContainer.classList.remove('opacity-40', 'pointer-events-none');
        }
    }
    if (previewOverlay) {
        if (isFull) {
...
window.toggleFullPosterMode = toggleFullPosterMode;
...
function handleBannerFileSelect(e) {
    ...
    reader.onload = (event) => {
        ...
        document.getElementById('form-banner-image').value = dataUrl;
        updateBannerPreviewImage(dataUrl);
    };
    reader.readAsDataURL(file);
}
...
function handleBannerUrlInput(url) {
    updateBannerPreviewImage(url.trim());
}
window.handleBannerUrlInput = handleBannerUrlInput;

function updateBannerPreviewImage(src) {
    const img = document.getElementById('form-banner-preview-img');
    const placeholder = document.getElementById('form-banner-empty-placeholder');
...
function clearBannerImage() {
    document.getElementById('form-banner-image').value = '';
    const fileInput = document.getElementById('form-banner-file');
    if (fileInput) fileInput.value = '';
...
function updateBannerLivePreview() {
    const heading = document.getElementById('form-banner-heading')?.value || 'Promotional Slide';
    const subtitle = document.getElementById('form-banner-subtitle')?.value || 'Add description';
    const pill = document.getElementById('form-banner-pill')?.value || '';
    const badge = document.getElementById('form-banner-badge')?.value || '';
    const btnText = document.getElementById('form-banner-cta-text')?.value || 'Order Now';

    const card = document.getElementById('form-banner-preview-card');
    const pTitle = document.getElementById('form-preview-title');
    const pSubtitle = document.getElementById('form-preview-subtitle');
    const pPill = document.getElementById('form-preview-pill');
    const pBadge = document.getElementById('form-preview-badge');
    const pBtn = document.getElementById('form-preview-btn-text');

    if (pTitle) pTitle.textContent = heading;
    if (pSubtitle) pSubtitle.textContent = subtitle;
    if (pPill) {
        pPill.textContent = pill;
        pPill.style.display = pill ? 'inline-block' : 'none';
    }
    if (pBadge) {
        pBadge.textContent = badge;
        pBadge.style.display = badge ? 'inline-block' : 'none';
    }
    if (pBtn) pBtn.textContent = btnText;

    const selectedGrad = document.querySelector('input[name="banner-gradient"]:checked')?.value || 'emerald';
    const gradColors = {
        emerald: 'linear-gradient(135deg, #064e3b 0%, #047857 50%, #10b981 100%)',
        purple: 'linear-gradient(135deg, #3b0764 0%, #581c87 50%, #7e22ce 100%)',
        amber: 'linear-gradient(135deg, #78350f 0%, #b45309 50%, #d97706 100%)',
        cyan: 'linear-gradient(135deg, #083344 0%, #0e7490 50%, #06b6d4 100%)',
        rose: 'linear-gradient(135deg, #4c0519 0%, #9f1239 50%, #e11d48 100%)',
        slate: 'linear-gradient(135deg, #0f172a 0%, #1e293b 50%, #334155 100%)'
    };
    if (card) {
        card.style.background = gradColors[selectedGrad] || gradColors.emerald;
    }
}
...
async function handleBannerSubmit(e) {
    ...
    const id = document.getElementById('form-banner-id').value;

    const isFullPoster = document.getElementById('form-banner-full-poster').checked;
    const title = document.getElementById('form-banner-heading').value.trim();
    const subtitle = document.getElementById('form-banner-subtitle').value.trim();
    const pill = document.getElementById('form-banner-pill').value.trim();
    const badge = document.getElementById('form-banner-badge').value.trim();
    const link_text = document.getElementById('form-banner-cta-text').value.trim();
    const link_url = document.getElementById('form-banner-cta-url').value.trim();
    const gradient = document.querySelector('input[name="banner-gradient"]:checked')?.value || 'emerald';
    const display_order = parseInt(document.getElementById('form-banner-order').value) || 1;
    const is_active = document.getElementById('form-banner-active').checked;
    const imageInput = document.getElementById('form-banner-image').value.trim();

    if (!isFullPoster && !title) {
    ...
    const payload = {
        title: title || (isFullPoster ? 'Promotional Poster' : ''),
        subtitle,
        pill,
        badge,
        link_text: link_text || 'Order Now',
        link_url: link_url || '#/categories',
        gradient,
        display_order,
        is_active,
        is_full_poster: isFullPoster
    };

    if (imageInput.startsWith('data:image/')) {
        payload.image_data = imageInput;
    } else {
        payload.image_url = imageInput;
    }

    try {
        const url = id ? `/api/banners/admin/${encodeURIComponent(id)}` : '/api/banners/admin';
        const method = id ? 'PUT' : 'POST';
        ...
        const data = await res.json();
        if (data.success) {
            showToast(`✓ Banner ${id ? 'updated' : 'created'} successfully`, 'success');
            closeBannerModal();
            await loadBannersAdmin();
        } else {
            showToast(data.error || 'Failed to save banner', 'warning');
        }
    } catch (err) {
        showToast('Error saving banner: ' + err.message, 'warning');
    } finally {
        if (saveBtn) {
            saveBtn.disabled = false;
            saveBtn.innerHTML = '<span class="material-symbols-outlined text-sm">save</span><span>Save Banner</span>';
        }
    }
}
...
async function toggleBannerStatus(id, currentStatus) {
    try {
        ...
        const res = await fetchWithTimeout(`/api/banners/admin/${encodeURIComponent(id)}`, {
            method: 'PUT',
            headers: getAuthHeaders(),
        ...
        const data = await res.json();
        if (data.success) {
            showToast(`Banner ${nextStatus ? 'activated' : 'paused'}`, 'info');
            await loadBannersAdmin();
        } else {
            showToast(data.error || 'Failed to update status', 'warning');
        }
    } catch (err) {
        showToast('Error toggling banner: ' + err.message, 'warning');
    }
}
window.toggleBannerStatus = toggleBannerStatus;

async function reorderBanner(id, direction) {
    const banners = [...adminBannersList].sort((a, b) => (a.display_order || 0) - (b.display_order || 0));
    ...
    // Swap display orders
    const currentOrder = banners[idx].display_order || (idx + 1);
    const targetOrder = banners[targetIdx].display_order || (targetIdx + 1);

    try {
        await Promise.all([
            fetchWithTimeout(`/api/banners/admin/${encodeURIComponent(banners[idx].id)}`, {
                method: 'PUT',
                headers: getAuthHeaders(),
                body: JSON.stringify({ display_order: targetOrder })
            }, 5000),
            fetchWithTimeout(`/api/banners/admin/${encodeURIComponent(banners[targetIdx].id)}`, {
                method: 'PUT',
                headers: getAuthHeaders(),
        ...
        showToast('Failed to update banner order: ' + err.message, 'warning');
    }
}
window.reorderBanner = reorderBanner;

async function handleModalDeleteBanner() {
    const id = document.getElementById('form-banner-id').value;
    if (!id) return;
    deleteBanner(id);
}
window.handleModalDeleteBanner = handleModalDeleteBanner;
...
async function deleteBanner(id) {
    ...
    try {
        ...
        if (data.success) {
            ...
            showToast(data.error || 'Failed to delete banner', 'warning');
        }
    } catch (err) {
        showToast('Error deleting banner: ' + err.message, 'warning');
    }
}
